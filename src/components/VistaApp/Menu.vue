<template>
<div>
    <v-navigation-drawer v-model="drawer" :mini-variant.sync="drawer" app expand-on-hover hide-overlay permanent clipped fluid absolute>
        <v-app-bar dark absolute permanent dense> </v-app-bar>

        <v-list nav dense margin-top="200" class="mt-12 mx-auto" :value="true" prepend-icon="mdi-account-circle">
            <v-list-item to="/">
                <v-list-item-icon>
            ...
            <v-list-group color="" no-action prepend-icon="mdi-file-document-multiple">
                ...
                <v-list>
                    <v-list-item v-if="$store.state.auth" @click="clickA()">
                        <v-list-item-icon>
                            <v-icon color="green">mdi-plus</v-icon>
                        </v-list-item-icon>
                        <v-list-item-title>Añadir categoria...</v-list-item-title>
                    </v-list-item>

                    <v-list-group v-for="item in items" :key="item.name" v-model="item.active" :prepend-icon="item.action" sub-group color="primary" active-class="pink--text">
                        <template v-slot:activator>
                            <v-list-item-content>
                        ...
                        <v-list-item v-if="$store.state.auth" @click="clickS(item.id)">
                            <v-list-item-icon>
                                <v-icon color="green">mdi-plus</v-icon>
                            </v-list-item-icon>
                            <v-list-item-title>Añadir sub Categoria...</v-list-item-title>
                        </v-list-item>
                    </v-list-group>
                </v-list>
...
export default {
    ...
    methods: {
        ...
        clickS(idC) {
            console.log('add SubCategory From Category ID ', idC);
        },
